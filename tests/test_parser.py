...
def test_binary_expression():
    assert DefaultParser().parse("1+2") == BinaryExpression(
        operator=_ops["+"], left=Literal(1), right=Literal(2)
    )
...
def test_binary_expression_encapsulation():
    assert DefaultParser().parse("2+3*4==5/6-7") == BinaryExpression(
        ...
        left=BinaryExpression(
            ...
            left=Literal(2),
            right=BinaryExpression(
                operator=_ops["*"], left=Literal(3), right=Literal(4)
            ),
        ),
        right=BinaryExpression(
            operator=_ops["-"],
            left=BinaryExpression(
                operator=_ops["/"], left=Literal(5), right=Literal(6)
            ),
            right=Literal(7),
        ),
    )
    ...
    assert DefaultParser().parse("1*!!true-2") == BinaryExpression(
        ...
        left=BinaryExpression(
            ...
            right=UnaryExpression(
                operator=_ops["!"],
                right=UnaryExpression(operator=_ops["!"], right=Literal(True)),
            ),
        ),
        right=Literal(2),
    )
...
def test_subexpression():
    assert DefaultParser().parse("(2+3)*4") == BinaryExpression(
        operator=_ops["*"],
        left=BinaryExpression(operator=_ops["+"], left=Literal(2), right=Literal(3)),
        right=Literal(4),
    )
...
def test_nexted_array_literals():
    assert DefaultParser().parse('["foo", ["bar", "tek"]]') == ArrayLiteral(
        [Literal("foo"), ArrayLiteral([Literal("bar"), Literal("tek")])]
    )
...
def test_chained_identifiers():
    assert DefaultParser().parse("foo.bar.baz + 1") == BinaryExpression(
        operator=_ops["+"],
        left=Identifier("baz", subject=Identifier("bar", subject=Identifier("foo"))),
        right=Literal(1),
    )
...
def test_functions_object_arg():
    assert DefaultParser().parse('fn1({bar:"tek"})') == Function(
        name="fn1", args=[ObjectLiteral({"bar": Literal("tek")})]
    )


def test_functions_multiple_arguments():
    assert DefaultParser().parse('fn1("bar","baz")') == Function(
        name="fn1", args=[Literal("bar"), Literal("baz")]
    )
    ...
    assert DefaultParser().parse('foo|tr1|tr2.baz|tr3({bar:"tek"})') == Transform(
        ...
        subject=Identifier(
            ...
            subject=Transform(
                name="tr2",
                args=[],
                subject=Transform(name="tr1", args=[], subject=Identifier("foo")),
            ),
        ),
    )
...
def test_transforms_multiple_arguments():
    assert DefaultParser().parse('foo|bar("tek", 5, true)') == Transform(
        name="bar",
        args=[Literal("tek"), Literal(5), Literal(True)],
...
def test_filters():
    assert DefaultParser().parse('foo[1][.bar[0]=="tek"].baz') == Identifier(
        ...
        subject=FilterExpression(
            ...
            subject=FilterExpression(
                relative=False, expression=Literal(1), subject=Identifier("foo")
            ),
        ),
    )
...
def test_attribute_all_operands():
    assert DefaultParser().parse('"foo".length + {foo: "bar"}.foo') == BinaryExpression(
        ...
        right=Identifier(value="foo", subject=ObjectLiteral({"foo": Literal("bar")})),
    )
...
def test_attribute_subexpression():
    assert DefaultParser().parse('("foo" + "bar").length') == Identifier(
        ...
        subject=BinaryExpression(
            operator=_ops["+"], left=Literal("foo"), right=Literal("bar")
        ),
    )
...
def test_attribute_array():
    assert DefaultParser().parse('["foo", "bar"].length') == Identifier(
        value="length", subject=ArrayLiteral([Literal("foo"), Literal("bar")])
    )
...
def test_nested_grouped_ternary_expression():
    assert DefaultParser().parse("foo ? (bar ? 1 : 2) : 3") == ConditionalExpression(
        ...
        consequent=ConditionalExpression(
            test=Identifier("bar"), consequent=Literal(1), alternate=Literal(2)
        ),
        alternate=Literal(3),
    )
...
def test_functions_in_ternary_expression():
    assert DefaultParser().parse('foo ? bar(1) : baz("tek")') == ConditionalExpression(
        test=Identifier("foo"),
        consequent=Function(name="bar", args=[Literal(1)]),
        alternate=Function(name="baz", args=[Literal("tek")]),
    )
